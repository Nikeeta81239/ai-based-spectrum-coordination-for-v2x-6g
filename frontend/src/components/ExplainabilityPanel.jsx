...
import { CheckCircle2, HelpCircle, Wifi, Gauge, Activity } from 'lucide-react';
...
export function ExplainabilityPanel({ explanation = null }) {
  ...
  const chDisplay = channel_label || (selected_channel !== undefined ? `CH${selected_channel + 1}` : '—');
  ...
  return (
    <div className="p-6 rounded-2xl border border-slate-800 glass-card space-y-6">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 font-mono mt-1">
            ...
            <span className="text-emerald-400 font-bold">
              {confidence === undefined ? '—' : formatPercent(confidence)}
            </span>
          </p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 font-mono text-xs">
        <div className="p-3 rounded-xl border border-slate-800 bg-slate-900/60 space-y-1 text-center">
          <div className="flex items-center justify-center gap-1 text-slate-400 uppercase text-[10px] tracking-wider">
            <Activity className="w-3 h-3" /> SINR
          </div>
          <div className="text-emerald-400 font-bold text-base">
            {sinr_db !== undefined ? `${formatNumber(sinr_db, 1)} dB` : '—'}
          </div>
        </div>
        <div className="p-3 rounded-xl border border-slate-800 bg-slate-900/60 space-y-1 text-center">
          <div className="flex items-center justify-center gap-1 text-slate-400 uppercase text-[10px] tracking-wider">
            <Wifi className="w-3 h-3" /> Interference
          </div>
          <div className="text-orange-400 font-bold text-base">
            {interference !== undefined ? formatPercent(interference) : '—'}
          </div>
        </div>
        <div className="p-3 rounded-xl border border-slate-800 bg-slate-900/60 space-y-1 text-center">
          <div className="flex items-center justify-center gap-1 text-slate-400 uppercase text-[10px] tracking-wider">
            <Gauge className="w-3 h-3" /> PDR
          </div>
          <div className="text-purple-400 font-bold text-base">
            {pdr !== undefined ? formatPercent(pdr) : '—'}
          </div>
        </div>
      </div>
